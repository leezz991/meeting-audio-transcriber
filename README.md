...
当前默认 PDF 布局已固化为“防跨页颜色块布局”：短章节左右成对排列，长章节和表格章节自动全宽，避免两栏长度不一致造成大面积空白。
...
- PDF 会关闭浏览器默认页眉页脚，不保留本地 HTML 文件地址、日期和页码等下角标信息。
...
- 最终纪要开头统一写：`处理方式：使用 meeting-audio-transcriber 技能整理。（作者LiuQ，github已开源）`，不在可见纪要中暴露 Whisper 模型名称。

## 维护与发布

后续如果对该技能进行优化、修复或扩展，默认流程是：
